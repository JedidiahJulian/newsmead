...
- Latest committed gaze checkpoint: `74f43c4` (`fix(gaze): align screen coordinates and document performance gate`). The hybrid shadow work described below remains uncommitted.
...
- Current decision gate: the lightweight BlazeFace/64x64-iris replacement branch is closed. Periodic coarse ROI failed twice on the SM (E-060); recursive and size-only refinements failed A56 generalization (E-062/E-063). One-step centre/size re-extraction passed the bounded A56 gate (E-065) but failed replicated SM horizontal geometry: candidate/intended horizontal correlation was 0.154 then 0.733 and candidate horizontal repeatability was only 0.513, despite complete re-extraction and strong vertical behavior (E-066). It must not control gaze or remain active in ordinary reading tests. The 478-point path remains authoritative.
...
Exact next action: checkpoint E-058-E-066 before the next architecture. Set `GAZE_HYBRID_SHADOW_ENABLED` false so ordinary calibration/reading runs use only the authoritative 478-point path, while retaining the uncommitted research harness and evidence. Do not stage `diagnostics-local/`, `tmp/`, or `tools/gaze/__pycache__/`; the official compact `face_landmark.tflite` downloaded under `tmp/` is not part of this checkpoint. After the user stages and commits the explicit hybrid-harness file list, add only a compute-only face-landmark benchmark to the researcher performance activity. Run it first on the SM-G991B with no camera, gaze output, calibration read/write, or participant run. Continue to a short three-position horizontal gate on both phones only if face-landmark inference leaves adequate room in a 50 ms frame budget for the existing two iris passes. Do not change the authoritative estimator, eye-local feature, mapper, filters, targets, order, timing, posture monitor, affine layer, or calibration data.
